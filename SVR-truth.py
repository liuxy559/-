...
# ====== 3. 启动 Optuna 优化 ======
save_dir = r"C:\Users\Lenovo\Desktop\v8\SVR"
...
plot_optimization_history(study).write_image(os.path.join(save_dir, "SVR_optuna_history_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_contour(study, params=['C', 'gamma']).write_image(os.path.join(save_dir, "SVR_contour_C_gamma_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_contour(study, params=['C', 'epsilon']).write_image(os.path.join(save_dir, "SVR_contour_C_epsilon_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_param_importances(study).write_image(os.path.join(save_dir, "SVR_param_importances_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_parallel_coordinate(study).write_image(os.path.join(save_dir, "SVR_parallel_coordinate_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_edf(study).write_image(os.path.join(save_dir, "SVR_edf_v8.png"), width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
...
# 训练最佳模型
best_model = SVR(**true_best_params)
...
# 保存模型文件
joblib.dump(best_model, os.path.join(save_dir, "best_SVR_model_v8.joblib"))
joblib.dump(scaler, os.path.join(save_dir, "best_SVR_scaler_v8.joblib"))
feature_names = X_trainval.columns.tolist()
joblib.dump(feature_names, os.path.join(save_dir, "best_SVR_features_v8.joblib"))
feature_medians = X_trainval.median()
joblib.dump(feature_medians, os.path.join(save_dir, "best_SVR_medians_v8.joblib"))

# ====== 6. 最终执行写入文件 ======
report_file_path = os.path.join(save_dir, "SVR-results_report-v8.txt")
...
ax.text(0.95, 0.05, f'Test $R^2$ = {r2_score(y_test, test_predict):.3f}', transform=ax.transAxes,
        fontsize=12, va='bottom', ha='right', bbox=dict(boxstyle="round", edgecolor="black", facecolor="white"))
ax.text(0.75, 0.99, 'Model = SVR', transform=ax.transAxes,
...
ax.legend()
plt.savefig(f"{save_dir}\\SVR-Seaborn联合图_v8.png",dpi=600)
plt.close()

# 折线对比图
plt.figure(figsize=(10, 6))
plt.plot(y_test.values, label='真实值', color='#1f77b4', linewidth=2)
plt.plot(test_predict, label='SVR预测值', color='#ff7f0e', linestyle='dashed', linewidth=2)
...
plt.tight_layout()
plt.savefig(f"{save_dir}\\SVR-折线对比图_v8.png",dpi=600)
plt.close()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(f"{save_dir}\\SVR-拟合散点图_v8.png",dpi=600)
plt.close()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(f"{save_dir}\\SVR-误差直方图_v8.png",dpi=600)
plt.close()
...
output_excel_path = f"{save_dir}\\SVR_预测结果_v8.xlsx"
...
print(f"\n所有 SVR-v8 结果图表和模型已保存至: {save_dir}")