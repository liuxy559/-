...
from sklearn.linear_model import ElasticNet  # <--- 1. 导入 ElasticNet
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
import optuna
import optuna.logging  # <====== 2. 导入 logging
import os              # <====== 3. 导入 os
...
# ====== 3. 启动 Optuna 优化 ======
save_dir = r"C:\Users\Lenovo\Desktop\v8\ElasticNet"
...
plot_optimization_history(study).write_image(f"{save_dir}\\ElasticNet_optuna_history_v8.png", width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_contour(study, params=['alpha', 'l1_ratio']).write_image(f"{save_dir}\\ElasticNet_contour_alpha_l1_ratio_v8.png", width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_param_importances(study).write_image(f"{save_dir}\\ElasticNet_param_importances_v8.png", width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_parallel_coordinate(study).write_image(f"{save_dir}\\ElasticNet_parallel_coordinate_v8.png", width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
plot_edf(study).write_image(f"{save_dir}\\ElasticNet_edf_v8.png", width=IMG_WIDTH, height=IMG_HEIGHT, scale=SCALE)
...
# 训练最佳模型
best_model = ElasticNet(**true_best_params)
...
# 保存模型文件
joblib.dump(best_model, f"{save_dir}\\best_ElasticNet_model_baseline_v8.joblib")
joblib.dump(scaler, f"{save_dir}\\best_ElasticNet_scaler_baseline_v8.joblib")
feature_names = X_trainval.columns.tolist()
joblib.dump(feature_names, f"{save_dir}\\best_ElasticNet_features_v8.joblib")
feature_medians = X_trainval.median()
joblib.dump(feature_medians, f"{save_dir}\\best_ElasticNet_medians_v8.joblib")

# ====== 6. 最终执行写入文件 ======
report_file_path = os.path.join(save_dir, "ElasticNet-results_report-v8.txt")
...
ax.text(0.95, 0.05, f'Test $R^2$ = {r2_score(y_test, test_predict):.3f}', transform=ax.transAxes,
        fontsize=12, va='bottom', ha='right', bbox=dict(boxstyle="round", edgecolor="black", facecolor="white"))
ax.text(0.75, 0.99, 'Model = ElasticNet', transform=ax.transAxes, # <--- 修改模型名称
...
ax.legend()
plt.savefig(f"{save_dir}\\ElasticNet-Seaborn联合图_v8.png",dpi=600)
plt.close()

# 折线对比图
plt.figure(figsize=(10, 6))
plt.plot(y_test.values, label='真实值', color='#1f77b4', linewidth=2)
plt.plot(test_predict, label='ElasticNet预测值', color='#ff7f0e', linestyle='dashed', linewidth=2) # <--- 修改标签
...
plt.tight_layout()
plt.savefig(f"{save_dir}\\ElasticNet-折线对比图_v8.png",dpi=600)
plt.close()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(f"{save_dir}\\ElasticNet-拟合散点图_v8.png",dpi=600)
plt.close()
...
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(f"{save_dir}\\ElasticNet-误差直方图_v8.png",dpi=600)
plt.close()
...
output_excel_path = f"{save_dir}\\ElasticNet预测结果_v8.xlsx"
...
print(f"\n所有 ElasticNet-v8 结果图表和模型已保存至: {save_dir}")